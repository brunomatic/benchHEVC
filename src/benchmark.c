...
void benchTransform(uint32_t numberOfIterations, uint8_t blockSize, uint8_t mode) 
{
	...
	random_residual = (int16_t *)malloc(sizeof(int16_t) * blockSize * blockSize);
...
void benchInterpolation(uint32_t numberOfIterations, uint8_t blockWidth, uint8_t blockHeight) 
{
	...
	for (i = 0; i < numberOfIterations; i++)
	{
		...
		generateRandomMV(&xPb, &yPb, (int16_t *)&mvLX, (int16_t *)&mvCLX, 1920, 1080, blockWidth, blockHeight);

		start_time = get_time();

		fractionalInterpolation(&picture, xPb, yPb, blockWidth, blockHeight, mvLX, mvCLX, 1, &Luma, &Cb, &Cr);
