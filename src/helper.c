...
void generatePicture(image * picture, uint32_t w, uint32_t h, uint8_t format) {
	...
	picture->Cb = (uint8_t *) malloc(
			sizeof(uint8_t) * h * w / picture->SubWidth);
	picture->Cr = (uint8_t *) malloc(
			sizeof(uint8_t) * h * w / picture->SubWidth);
	...
	for (y = 0; y < (h / picture->SubHeight); y++) {
		for (x = 0; x < (w / picture->SubWidth); x++) {
			picture->Cb[x + y * (picture->width / picture->SubWidth)] = (x * 4
					* picture->SubWidth) % 256;
			picture->Cr[x + y * (picture->width / picture->SubWidth)] = (x * 4
					* picture->SubWidth) % 256;
 ...
 */
void printPlaneSegment(image * img, uint8_t sizeX, uint8_t sizeY, uint32_t x,
		uint32_t y, uint8_t component) {
	uint32_t i, j;
 ...
 */
void generateRandomMV(uint32_t * xPb, uint32_t * yPb, int16_t * mvLX,
		int16_t * mvCLX, uint32_t width, uint32_t height, uint8_t blockWidth,
		uint8_t blockHeight) {
	srand(time(NULL));
