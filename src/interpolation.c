...
*/
const int8_t interpolationCoeffLuma[3][8] =  \
{
	{-1, 4, -10, 58, 17, -5,  1,  0},
	{-1, 4, -11, 40, 40, -11, 4, -1},
	{ 0, 1,  -5, 17, 58, -10, 4, -1}
...
*/
const int8_t interpolationCoeffChroma[7][4] = \
{
	{-2, 58, 10, -2},
	{-4, 54, 16, -2},
	{-6, 46, 28, -4},
	...
	predictionSample * Y, predictionSample * Cb, predictionSample * Cr)
{
	...
	for (y = 0; y  < nPbH; y ++)
	{
		for (x = 0; x < nPbW;x ++) {
			...
			Y->data[x + y * Y->width] = interpolateLumaSample(xInt + x, yInt + y, xFract, yFract, picture);
	...
	if (ChromaArrayType) {
		...
		Cb->data = (int16_t *)malloc(sizeof(int16_t) * (nPbH/picture->SubHeight) * (nPbW/picture->SubWidth));
		Cb->width = nPbW / picture->SubWidth;
		Cb->height = nPbH / picture->SubHeight;

		Cr->data = (int16_t *)malloc(sizeof(int16_t) * (nPbH / picture->SubHeight) * (nPbW / picture->SubWidth));
		...
		for (y = 0; y  < (nPbH / picture->SubHeight); y ++)
		{
			for (x = 0; x < (nPbW / picture->SubWidth); x++) {
				...
				interpolateChromaSample(xInt + x, yInt + y, xFract, yFract, picture, &(Cb->data[x + y * Cb->width]), &(Cr->data[x + y * Cr->width]));
...
*/
int16_t interpolateLumaSample(int32_t x, int32_t y, uint8_t xFract, uint8_t yFract, image * picture)
{
	...
	else 
	{
		...
		// those subpixels are retreived by recursive call with different coordinates
			...
			for (i = 0; i < 8; i++)
			{
				samples[i] = interpolateLumaSample(x, y - 3 + i, xFract, 0, picture);
			}
			shift = 6;
		}
	}
	...
	*CrSample = 0;
	...
	else
	{
		...
		else
		{
			for (i = 0; i < 4; i++)
			{
				interpolateChromaSample(x, y - 1 + i, yFract, 0, picture, &samples[0][i], &samples[1][i]);
			}
			shift = 6;
			rowIndex = yFract - 1;
		}
